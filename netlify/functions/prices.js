...
async function fetchJustETF(isin) {
  ...
  const price = data?.latestQuote?.price ?? data?.price ?? data?.nav;
  const changePercent = data?.latestQuote?.changePercent ?? data?.changePercent ?? 0;
  if (!price || isNaN(price) || price <= 0) {
    console.log(`JustETF ${isin}: no price in response`, JSON.stringify(data).slice(0, 200));
    return null;
  }
  return { price: parseFloat(price.toFixed(4)), dayChange: parseFloat((changePercent || 0).toFixed(2)) };
}
